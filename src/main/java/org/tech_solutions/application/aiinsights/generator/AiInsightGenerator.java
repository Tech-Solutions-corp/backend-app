...
public class AiInsightGenerator {
    ...
    private String buildPrompt(
        ...
        for (Object[] row : incomeVsExpense) {
            ...
            if (type == TransactionType.INCOME)  incomeFormatted  = "R$%.2f".formatted(total);
            if (type == TransactionType.EXPENSE) expenseFormatted = "R$%.2f".formatted(total);
        }
        String incomeVsExpenseFormatted = "Receita: %s | Despesa: %s".formatted(incomeFormatted, expenseFormatted);
        ...
        String topExpensesFormatted = topExpenses.stream()
                .map(t -> "%s R$%.2f (%s)".formatted(
                        t.getTransactionDescription() != null ? t.getTransactionDescription() : "Sem descrição",
                        t.getAmount(),
